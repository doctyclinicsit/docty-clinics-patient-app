...
const LAB_TEST_SNAPSHOT_PATH = 'lab-tests/latest.json';
const LAB_TEST_SNAPSHOT_MAX_AGE_MS = 2 * 60 * 60 * 1000;
...
function fallbackSnapshot() {
  return {
    ...
    source: 'fallback',
    catalogVersion: LAB_TEST_CATALOG_VERSION,
    testCount: labTestCatalog.length,
    categoryCount: labTestCategories.length,
    tests: labTestCatalog,
    categories: labTestCategories,
  } satisfies LabTestsSnapshot;
}

export function filterLabTests(tests: LabTestItem[], request: any) {
...
export function isFreshLabTestsSnapshot(snapshot: LabTestsSnapshot | null): snapshot is LabTestsSnapshot {
  if (!snapshot?.syncedAt || !Array.isArray(snapshot.tests)) return false;
  if (snapshot.catalogVersion !== LAB_TEST_CATALOG_VERSION) return false;
  const syncedAt = new Date(snapshot.syncedAt).getTime();
  return Number.isFinite(syncedAt) && Date.now() - syncedAt <= LAB_TEST_SNAPSHOT_MAX_AGE_MS;
}

export async function readLabTestsSnapshot() {
  const blobToken = process.env.BLOB_READ_WRITE_TOKEN;
  if (!blobToken) return null;

  const blob = await get(LAB_TEST_SNAPSHOT_PATH, {
    access: 'private',
    token: blobToken,
  }).catch(() => null);
  ...
  const snapshot = JSON.parse(text || 'null') as LabTestsSnapshot | null;
  if (!snapshot || !Array.isArray(snapshot.tests)) return null;
  return snapshot;
}

export async function writeLabTestsSnapshot(snapshot: LabTestsSnapshot) {
  const blobToken = process.env.BLOB_READ_WRITE_TOKEN;
  if (!blobToken) throw new Error('Lab tests snapshot storage is not configured.');

  await put(LAB_TEST_SNAPSHOT_PATH, JSON.stringify(snapshot), {
    access: 'private',
...
export async function fetchLabTestsSnapshot() {
  const databaseUrl = getDatabaseUrl();
  if (!databaseUrl) return fallbackSnapshot();
  ...
  `;
  const tests = normalizeRows(rows as LabTestRow[]);
  const categories = [
...
export async function refreshLabTestsSnapshot() {
  const snapshot = await fetchLabTestsSnapshot();
  await writeLabTestsSnapshot(snapshot);
  return snapshot;
}

export function labTestsFallbackResponse(request: any, source: 'fallback' | 'fallback-error', error?: unknown) {
  const snapshot = fallbackSnapshot();
  return {
    tests: filterLabTests(snapshot.tests, request),
